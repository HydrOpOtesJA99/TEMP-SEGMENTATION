...
checkpoint_dir = base_dir + "/" + train_folder + "/" + MODEL +"_LEAF_SEG_MODEL_checkpoints_" + current_time_str
...
def json_to_mask_leaf(json_path, shape):
...
class ModelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        H, W = Img.shape[:2]
        mask = json_to_mask_leaf(ent["annotation"], shape=(H,W))
        Img = cv2.resize(Img, (self.image_size, self.image_size), interpolation=cv2.INTER_CUBIC) # https://doi.org/10.1007/s11760-024-03421-9
        mask = cv2.resize(mask, (self.image_size, self.image_size), interpolation=cv2.INTER_CUBIC) # https://doi.org/10.1007/s11760-024-03421-9
...
def compute_iou_per_image(preds, targets, cls):
    ...
    for p, t in zip(preds, targets):
        ...
        if t_cls.sum() == 0 and p_cls.sum() == 0:  # target, pred 모두 없음
            ious.append(1.0)
        elif t_cls.sum() == 0:  # target 없음, pred 있음
            ious.append(0.0)
        else:
...
def compute_dice_per_image(preds, targets, cls):
    ...
    dices = []
    for p, t in zip(preds, targets):
        preds_cls = (p == cls).float()
        targets_cls = (t == cls).float()
...
for epoch in range(NUM_EPOCHS):
    ...
    for cls, name in zip(list(range(classes_value)), ["BG","Leaf"]):
        print(f"Class {name}:")
    ...
    # =========================
    # 3) 최고 Leaf IoU 기준 checkpoint 저장
    # =========================
    leaf_class_iou = avg_val_metrics[1]["iou"]  # 클래스 1 = Leaf
    if leaf_class_iou > best_leaf_iou:
        best_leaf_iou = leaf_class_iou
        leaf_ckpt_path = os.path.join(
            checkpoint_dir,
            f"checkpoint_best_leafIoU_{current_time_str}.pth"
        )
        torch.save(checkpoint, leaf_ckpt_path)
        print(f"Best-Leaf-IoU checkpoint saved (val_leaf_IoU={best_leaf_iou:.4f}")


#     # =========================
#     # EarlyStopping
#     # =========================
#     earlystop.step(avg_iou_leaf, model)
...
plt.xlabel("Epoch"); plt.ylabel("IoU"); plt.title("Leaf IoU"); plt.legend()
plt.ylim(0, 1)
...
plt.xlabel("Epoch"); plt.ylabel("Dice"); plt.title("Leaf Dice"); plt.legend()
plt.ylim(0, 1)
...
metrics_df = pd.DataFrame({
    ...
    # 클래스별
    "Train_Leaf_IoU": log_metrics["train"][1]["iou"],
    "Val_Leaf_IoU": log_metrics["val"][1]["iou"],
    "Train_Leaf_Dice": log_metrics["train"][1]["dice"],
    "Val_Leaf_Dice": log_metrics["val"][1]["dice"],
    "Train_Leaf_Accuracy": log_metrics["train"][1]["acc"],
    "Val_Leaf_Accuracy": log_metrics["val"][1]["acc"],
    "Train_Leaf_Precision": log_metrics["train"][1]["prec"],
    "Val_Leaf_Precision": log_metrics["val"][1]["prec"],
    "Train_Leaf_Recall": log_metrics["train"][1]["rec"],
    "Val_Leaf_Recall": log_metrics["val"][1]["rec"],
    "Train_Leaf_F1": log_metrics["train"][1]["f1"],
    "Val_Leaf_F1": log_metrics["val"][1]["f1"],
...
# =========================
alpha = 0.5
classes = [0, 1, 2]           # 0: BG(Leaf), 1: Disease
ignore_index = 255

class_colors = {
    0: [0, 255, 0],  # Leaf/BG: Light gray
    1: [0, 255, 0], 
    2: [192, 192, 192]
}
...
path_parameter = "_LEAF_SEG_MODEL_checkpoints_20260106_192352"
...
best_loss_ckpt_path = os.path.join(
    ...
    "checkpoint_best_leafIoU_20260106_192352.pth"
)
...
with torch.no_grad():
    for batch_idx, (images, masks) in enumerate(valid_loader):
        ...
        for i in range(batch_size):
            ...
            gt_leaf_total = (gt_mask == 0) | (gt_mask == 1)
            gt_color[gt_leaf_total] = [0, 255, 0]   # Leaf 초록
            gt_color[gt_mask == 1] = [0, 255, 0]    # Disease 빨강
            gt_color[gt_mask == 2] = [192, 192, 192] # BG 회색

            # -------------------------
            # Prediction overlay
            # -------------------------
            pred_bg = (pred_mask == 0)
            pred_leaf = (pred_mask == 1)

            overlay = image.copy()
            color_mask = np.zeros((H, W, 3), dtype=np.uint8)
            color_mask[pred_bg] = [0, 0, 0]
            color_mask[pred_leaf] = [0, 255, 0]
            overlay = cv2.addWeighted(overlay, 1-alpha, color_mask, alpha, 0)

            # -------------------------
            # IoU / Dice 계산 (GT leaf+lesion vs pred leaf)
            # -------------------------
            pred_leaf = (pred_mask == 1) 
            gt_leaf_total = (gt_mask == 0) | (gt_mask == 1)  # GT leaf + disease
            inter = np.logical_and(pred_leaf, gt_leaf_total & valid_region).sum()
            union = np.logical_or(pred_leaf, gt_leaf_total & valid_region).sum()
            iou = inter / union if union > 0 else 1.0

            pred_sum = pred_leaf.sum()  # sangjin update
            gt_sum   = (gt_leaf_total & valid_region).sum()
            dice = (2 * inter) / (pred_sum + gt_sum) if (pred_sum + gt_sum) > 0 else 1.0

            # Metric 저장
            class_iou_list[1].append(iou)   # Leaf
            class_dice_list[1].append(dice) # Leaf

            if matched_prefix is not None:
                prefix_results[matched_prefix][1].setdefault("iou", []).append(iou)
                prefix_results[matched_prefix][1].setdefault("dice", []).append(dice)

            # -------------------------
            # 이미지별 metric 출력
            # -------------------------
            print(f"[{global_idx+1}] {img_name} | Leaf IoU: {iou:.4f}, Dice: {dice:.4f}")
...
print("======================")
for c, name in zip(classes, ["BG", "Leaf"]):
    print(
